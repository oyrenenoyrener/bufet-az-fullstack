...
export default function RegisterPage() {
    ...
    // 1. State (Yaddaş qutuları)
    const [formData, setFormData] = useState({
        phone_number: "",
        password: "",
        fin_code: "", // <--- Bu mütləq olmalıdır
    });
    ...
    // 2. Yazı yazanda işləyən funksiya
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setFormData({ ...formData, [e.target.name]: e.target.value });
    };

    // 3. Şəkil seçəndə işləyən funksiya
    ...
    // 4. "Qeydiyyat" düyməsi (ƏSAS HİSSƏ)
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 👇 DÜZƏLİŞ BURADADIR: FİN kodu mütləq əlavə edirik 👇
        data.append("fin_code", formData.fin_code);
        ...
        try {
            ...
            setTimeout(() => router.push("/login"), 2000); // Qeydiyyatdan sonra girişə atırıq

        } catch (error: any) {
            console.error("Xəta:", error);
            if (error.response) {
                // Serverdən gələn dəqiq xətanı göstəririk
            ...
            <div className="w-full max-w-md bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            name="fin_code" // <--- Adı dəqiq olmalıdır
                    ...
                    <div className="text-center mt-4">
                        <Link href="/login" className="text-sm text-slate-400 hover:text-white">
                            Artıq hesabınız var? <span className="text-[#FF6B00]">Daxil olun</span>
                        </Link>
                    </div>
                </form>
            </div>
